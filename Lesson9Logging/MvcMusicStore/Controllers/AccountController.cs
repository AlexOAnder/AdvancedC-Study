...
namespace MvcMusicStore.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
					...
					//Set counter
					CounterHelper.counterHelper.Increment(Counters.LogIn);
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
					...
					//Save info about registered user in log
					logger.Debug("User {0} registered", model.UserName);
        ...
        public ActionResult LogOff()
        {
			...
			//Set counter
			CounterHelper.counterHelper.Increment(Counters.LogOut);
			//Save in log info about log out
			logger.Info("User {0} logged off", AuthenticationManager.User.Identity.Name);
			return RedirectToAction("Index", "Home");
        }
