namespace ORM.Tests
{
    ...
    public class UnitTest
    {
		...
		// task 2
        [TestMethod]
        public void ListOfProductsWithCategoryAndSupplier() // 2.1
        {
        ...
        [TestMethod]
        public void ExecuteProcedureThatRepresentEmployeeSalesByCountry() // 2.3
        {
            using (var db = new SqlConnection(this.connectionString))
            {
        ...
        [TestMethod]
        public void EmployeeWithShipper() //2.4
        {
            using (var db = new SqlConnection(this.connectionString))
            {
                var orders = db.Query<Order>("SELECT * FROM dbo.Orders").ToList();
